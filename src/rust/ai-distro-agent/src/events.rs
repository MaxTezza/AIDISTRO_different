...
async fn monitor_hardware(tx: mpsc::Sender<SystemEvent>) -> Result<(), Box<dyn Error>> {
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or("Failed to capture udevadm stdout")?;
    ...
    while let Ok(Some(line)) = reader.next_line().await {
        if line.contains("ID_MODEL=") {
            let model = line.split('=').next_back().unwrap_or("Unknown Device");
    ...
    while let Some(event) = rx.recv().await {
        let message = match event {
            SystemEvent::BatteryLow(pct) => {
                format!(
                    "Battery is low ({}%). Should I enable power saving mode?",
                    pct
                )
            }
            ...
            SystemEvent::HardwareAdded(dev) => {
                format!(
                    "I see a new device: {}. Should I index its files or help you open it?",
                    dev
                )
            }
