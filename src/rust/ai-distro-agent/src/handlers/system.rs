        ...
        match run_command("flatpak", &["update", "-y"], None) {
            Ok(_) => {
                ...
                ok_response(
                    &req.name,
                    &format!(
                        "I have finished updating your system and apps on the {channel} channel."
                    ),
                )
            }
            Err(err) => error_response(
                &req.name,
                &format!(
                    "I finished updating your system. Flatpak apps could not be updated: {err}"
                ),
            ),
        }
    } else {
...
pub fn handle_self_update(req: &ActionRequest) -> ActionResponse {
    ...
    match std::process::Command::new("cargo")
        ...
        .output()
    {
        Ok(out) if out.status.success() => ok_response(
            &req.name,
            "I've updated my own code and rebuilt my core. Please restart me to apply changes.",
        ),
        Ok(out) => error_response(
            &req.name,
            &format!("Build failed: {}", String::from_utf8_lossy(&out.stderr)),
        ),
...
pub fn handle_import_legacy_data(req: &ActionRequest) -> ActionResponse {
    ...
    // Run asynchronously
    let _ = Command::new(crate::utils::resolve_venv_python())
        .arg(tool)
        .arg(path)
        .spawn();
