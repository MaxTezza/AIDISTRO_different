...
pub fn handle_set_dark_mode(req: &ActionRequest) -> ActionResponse {
    ...
    if success {
        ...
        error_response(
            &req.name,
            "Failed to set dark mode. Your desktop environment may not support this action.",
        )
    }
}
...
pub fn handle_set_light_mode(req: &ActionRequest) -> ActionResponse {
    ...
    let success = match de.as_str() {
        ...
        "kde" | "plasma" => run_command("plasma-apply-colorscheme", &["BreezeLight"], None).is_ok(),
        "sway" | "hyprland" => run_command(
            "gsettings",
            &["set", "org.gnome.desktop.interface", "gtk-theme", "Adwaita"],
            None,
        )
    ...
    if success {
        ...
        error_response(
            &req.name,
            "Failed to set light mode. Your desktop environment may not support this action.",
        )
    }
}
    ...
    for part in payload.split(',') {
        ...
        if kv.len() >= 2 {
            match kv[0].to_lowercase().as_str() {
                "light" => {
                    schedule.light_mode_start = format!(
                        "{}:{}",
                        kv.get(1).unwrap_or(&"07"),
                        kv.get(2).unwrap_or(&"00")
                    );
                }
                "dark" => {
                    schedule.dark_mode_start = format!(
                        "{}:{}",
                        kv.get(1).unwrap_or(&"19"),
                        kv.get(2).unwrap_or(&"00")
                    );
                }
                "nightlight" => {
                    schedule.night_light_enabled = true;
                    schedule.night_light_start = format!(
                        "{}:{}",
                        kv.get(1).unwrap_or(&"20"),
                        kv.get(2).unwrap_or(&"00")
                    );
                }
                _ => {}
...
pub fn handle_apply_scheduled_theme(req: &ActionRequest) -> ActionResponse {
    ...
    if should_be_dark && schedule.current_theme != "dark" {
        ...
        ok_response(
            &req.name,
            &format!(
                "Theme already set correctly to {} mode.",
                schedule.current_theme
            ),
        )
    }
}
