...
pub fn handle_ui_click(req: &ActionRequest) -> ActionResponse {
    ...
    if element_name.contains(',') {
        ...
        if parts.len() == 2 {
            let x = parts[0];
            let y = parts[1];
            if let Ok(_) = run_command("xdotool", &["mousemove", x, y, "click", "1"], None) {
    ...
    match run_command("xdotool", &["search", "--name", element_name, "windowactivate"], None) {
        Ok(_) => ok_response(&req.name, &format!("Focused and clicked element '{}'", element_name)),
...
pub fn handle_ui_type(req: &ActionRequest) -> ActionResponse {
    ...
    if parts.len() == 2 {
        ...
        let _ = run_command("xdotool", &["search", "--name", win, "windowactivate"], None);
