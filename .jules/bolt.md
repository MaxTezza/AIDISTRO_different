...
## 2024-05-18 - [Batching SQLite Inserts]
**Learning:** Using `conn.execute` in a loop for database inserts creates a severe N+1 bottleneck, a common anti-pattern in database interactions within this codebase.
**Action:** Always batch database inserts, especially ON CONFLICT updates for term frequencies, using `conn.executemany` to minimize overhead and improve indexing performance.
