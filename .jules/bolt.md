...
## 2026-06-15 - Token Frequency list.count Optimization
**Learning:** When calculating term frequencies for a small subset of query terms against a large document list in Python, iterating over the query terms and using `list.count(term)` is significantly faster (around ~50% improvement) than building a full `collections.Counter()` for the document, avoiding unnecessary token hashing overhead.
**Action:** When calculating overlap for sparse queries against tokenized text, prioritize iterating query terms with `list.count()` rather than full document hashing.
