...
## 2026-06-16 - Batching SQLite Inserts to avoid N+1 Bottleneck
**Learning:** In `tools/agent/file_intelligence.py` and `tools/agent/conversation_memory.py`, updating term frequencies inside a Python loop using `conn.execute` causes significant overhead.
**Action:** Use `conn.executemany` to batch `INSERT INTO doc_freq ... ON CONFLICT` operations, avoiding severe N+1 SQLite bottlenecks compared to looping with `conn.execute`.
