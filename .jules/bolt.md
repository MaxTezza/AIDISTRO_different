...
## 2024-05-18 - [Fix N+1 query and redundant calculation bottleneck in Conversation Memory TF-IDF search]
**Learning:** In the SQLite-backed TF-IDF semantic search implementation (`conversation_memory.py`), loading the entire `doc_freq` vocabulary table into memory becomes a massive bottleneck as the corpus scales (e.g., from 0.19s to 1.95s recall time at 1M terms). Also, calculating the query vector magnitude (`mag_q`) repeatedly inside the document scoring loop adds redundant O(N) overhead.
**Action:** Use a two-pass approach: 1) fetch documents and identify only the specific vocabulary terms needed for matching documents, then 2) query `doc_freq` with batched `IN (...)` queries chunked at 999. Also, precalculate loop invariants like query magnitude (`mag_q`) outside of the scoring loop, which, when combined with inlining cosine similarity, results in >100x speedup for large vocabularies.
