...
## 2026-06-25 - TF-IDF Pre-calculation Cache Memory Leak

**Learning:** In local SQLite RAG systems (like `tools/agent/conversation_memory.py`), eagerly fetching the entire vocabulary frequency table (`SELECT term, count FROM doc_freq`) into memory to avoid N+1 queries scales poorly as the database grows, inflating memory usage and load times unnecessarily.
**Action:** Use a two-pass approach: first iterate through documents to identify matching ones via a disjoint set check and collect their unique terms, then perform a single batched `IN` query to fetch frequencies *only* for the necessary terms.
