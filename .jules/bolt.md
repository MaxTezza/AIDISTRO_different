...
## 2026-07-13 - TF-IDF Full Vocabulary Loading Bottleneck
**Learning:** When trying to resolve an SQLite N+1 bottleneck, loading an entire reference table (like `SELECT term, count FROM doc_freq`) into memory can introduce severe overhead and memory spikes when the vocabulary grows large.
**Action:** Use a two-pass approach. First, scan target documents for the specific terms you need. Second, use batched `IN` queries (chunked by 999 to respect SQLite limits) to fetch the reference data *only* for those required terms. This avoids N+1 queries while maintaining low memory footprint.
