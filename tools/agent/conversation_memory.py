    ...
    def recall(self, query, top_k=5):
        ...
        for row in rows:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            matched_rows.append((row, doc_tokens, False))
        ...
        for note in notes:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            matched_rows.append((note, doc_tokens, True))
            all_matched_terms.update(doc_tokens)

        # ⚡ Bolt: Fetch only document frequencies for terms in matching documents to prevent loading massive vocabulary into memory
        df_cache = {}
        all_terms_list = list(all_matched_terms)
        ...
        scored = []
        for row, doc_tokens, is_note in matched_rows:
