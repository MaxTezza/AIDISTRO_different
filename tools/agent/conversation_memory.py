...
class ConversationMemory:
    ...
    def _compute_tfidf(self, tokens, conn, num_docs=None, doc_freq_cache=None):
        ...
        # Get total document count if not provided
        if num_docs is None:
    ...
    def recall(self, query, top_k=5):
        ...
        # Performance optimization: cache document frequencies and total document count
        # to avoid N+1 SQLite queries during scoring loop.
        num_docs_row = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(num_docs_row[0], 1)
        df_cache = {}

        query_vec = self._compute_tfidf(query_tokens, conn, num_docs, df_cache)
        ...
        for row in rows:
            ...
            if not doc_tokens:
                continue
            doc_vec = self._compute_tfidf(doc_tokens, conn, num_docs, df_cache)
