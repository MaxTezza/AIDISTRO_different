...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # Gather all required terms to avoid full table scan and N+1 queries
        ...
        for row in rows:
            ...
            valid_rows.append((row, doc_tokens))
        ...
        for note in notes:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            required_terms.update(doc_tokens)
            valid_notes.append((note, doc_tokens))

        # Fetch required document frequencies in chunks
        ...
        # Calculate query vector now that we have df_cache
        ...
        scored = []
        for row, doc_tokens in valid_rows:
        ...
        # Also search notes
        for note, doc_tokens in valid_notes:
            doc_vec = self._compute_tfidf(doc_tokens, conn, num_docs=num_docs, df_cache=df_cache)
            sim = self._cosine_similarity(query_vec, doc_vec, mag_a=mag_q)
            if sim > 0.05:
                scored.append({
