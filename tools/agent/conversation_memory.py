...
class ConversationMemory:
    ...
    def _compute_tfidf(self, tokens, conn, num_docs=None, df_cache=None):
        ...
        Args:
            ...
            num_docs: Optional total document count (for batched operations)
            df_cache: Optional dictionary mapping term to document frequency (for batched operations)
        """
        tf = Counter(tokens)
    ...
    def recall(self, query, top_k=5):
        ...
        # Performance optimization: bulk fetch document frequencies and count to avoid N+1 queries
        num_docs_row = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(num_docs_row[0], 1)
        df_cache = dict(conn.execute("SELECT term, count FROM doc_freq").fetchall())
