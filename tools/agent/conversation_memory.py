...
class ConversationMemory:
    ...
    def _compute_tfidf(self, tokens, conn, num_docs=None, df_cache=None):
        ...
        Accepts optional num_docs and df_cache to avoid N+1 queries during batch processing."""
        tf = Counter(tokens)
    ...
    def recall(self, query, top_k=5):
        ...
        # Pre-fetch num_docs and document frequencies to avoid N+1 queries during scoring
        row = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(row[0], 1)

        all_terms = conn.execute("SELECT term, count FROM doc_freq").fetchall()
        df_cache = {term: count for term, count in all_terms}
