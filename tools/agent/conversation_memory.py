...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # Cache document frequency and total documents to avoid N+1 queries
        df_cache = dict(conn.execute("SELECT term, count FROM doc_freq").fetchall())
        row_count = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(row_count[0], 1)
