...
class ConversationMemory:
    ...
    def _compute_tfidf(self, tokens, conn, num_docs=None, doc_freq_cache=None):
    ...
    def recall(self, query, top_k=5):
        ...
        # Performance optimization: pre-fetch total document count and term frequencies
        # to prevent N+1 query problem during TF-IDF computation in loop
        doc_count_row = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(doc_count_row[0], 1)

        doc_freq_rows = conn.execute("SELECT term, count FROM doc_freq").fetchall()
        doc_freq_cache = {row[0]: row[1] for row in doc_freq_rows}

        query_vec = self._compute_tfidf(query_tokens, conn, num_docs, doc_freq_cache)
        ...
        for row in rows:
            ...
            if not doc_tokens:
                continue
            doc_vec = self._compute_tfidf(doc_tokens, conn, num_docs, doc_freq_cache)
