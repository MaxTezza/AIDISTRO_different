...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # TWO-PASS APPROACH: Find matching documents first to collect required terms
        matched_docs = []
        ...
        for row in rows:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            matched_docs.append(("conv", row, doc_tokens))
        ...
        for note in notes:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            matched_docs.append(("note", note, doc_tokens))
            required_terms.update(doc_tokens)

        # Load ONLY required doc frequencies into memory using batched IN queries
        df_cache = {}
        term_list = list(required_terms)
        for i in range(0, len(term_list), 999):
            chunk = term_list[i:i+999]
        ...
        scored = []
        for doc_type, row, doc_tokens in matched_docs:
            doc_vec = self._compute_tfidf(doc_tokens, conn, num_docs=num_docs, df_cache=df_cache)
            # Use pre-calculated query magnitude for cosine similarity
            # Replace inner _cosine_similarity call to avoid O(N) redundant calculations of mag_q
