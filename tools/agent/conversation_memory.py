...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # Pass 1: Identify matching documents and collect needed terms
        ...
        for row in rows:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            matching_convos.append((row, doc_tokens))
        ...
        # Pass 2: Load document frequencies ONLY for needed terms (chunked to avoid SQLite limits)
        df_cache = {}
        needed_list = list(needed_terms)
        for i in range(0, len(needed_list), 999):
            chunk = needed_list[i:i+999]
        ...
        scored = []
        for row, doc_tokens in matching_convos:
