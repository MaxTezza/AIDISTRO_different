...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # OPTIMIZATION: Fetch only required doc frequencies to avoid full table load
        num_docs_row = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(num_docs_row[0], 1)

        query_terms = set(query_tokens)

        # 1. Fetch potential matching documents and collect terms
        rows = conn.execute(
        ...
        # 2. Fetch doc frequencies only for the needed terms (chunked by 999 for SQLite limits)
        df_cache = {}
