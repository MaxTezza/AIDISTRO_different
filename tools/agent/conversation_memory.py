...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # Load doc frequencies into memory using a two-pass approach to avoid fetching the entire vocabulary
        # First pass: identify overlapping documents and their terms
        ...
        for row in rows:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            relevant_docs.append(("conv", row, doc_tokens))
        ...
        for note in notes:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            relevant_docs.append(("note", note, doc_tokens))
            all_relevant_terms.update(doc_tokens)

        # Fetch ONLY the doc frequencies we need
        df_cache = {}
        term_list = list(all_relevant_terms)
        ...
        scored = []
        for doc_type, row, doc_tokens in relevant_docs:
