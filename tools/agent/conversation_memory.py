...
class ConversationMemory:
    ...
    def _compute_tfidf(self, tokens, conn, num_docs=None, df_cache=None):
        ...
        Args:
            ...
            num_docs: Optional pre-computed document count (optimization)
            df_cache: Optional dictionary of term document frequencies (optimization)
        """
        tf = Counter(tokens)
    ...
    def recall(self, query, top_k=5):
        ...
        # ⚡ Bolt Optimization: Pre-fetch doc_freq and num_docs to avoid N+1 queries during TF-IDF calculation
        row = conn.execute("SELECT COUNT(*) FROM conversations").fetchone()
        num_docs = max(row[0], 1)

        # Load the document frequencies into a memory dictionary to skip thousands of small SQLite queries
        df_cache = {row[0]: row[1] for row in conn.execute("SELECT term, count FROM doc_freq").fetchall()}
