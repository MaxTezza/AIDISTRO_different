...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # Get matching documents and their terms to optimize vocabulary loading
        rows = conn.execute(
        ...
        # Gather all terms only from documents that share at least one term with the query
        ...
        for row in rows:
            ...
            if doc_tokens and not query_terms.isdisjoint(doc_tokens):
                matching_convs.append((row, doc_tokens))
        ...
        # Load only required doc frequencies into memory
        df_cache = {}
        ...
        scored = []
        for row, doc_tokens in matching_convs:
