...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # 1. Fetch matching conversations and collect their unique terms
        rows = conn.execute(
        ...
        matched_rows = []
        doc_tokens_list = []
        all_relevant_terms = set(query_tokens)
        ...
        for row in rows:
            ...
            all_relevant_terms.update(doc_tokens)

        # 2. Fetch matching notes
        notes = conn.execute(
        ...
        for note in notes:
            ...
            note_tokens_list.append(doc_tokens)
            all_relevant_terms.update(doc_tokens)
        ...
        # 3. Load ONLY relevant doc frequencies into memory to avoid fetching huge vocabulary
        df_cache = {}
        all_relevant_terms_list = list(all_relevant_terms)
        for i in range(0, len(all_relevant_terms_list), 999):
            chunk = all_relevant_terms_list[i:i+999]
        ...
        scored = []
        for row, doc_tokens in zip(matched_rows, doc_tokens_list):
        ...
        for note, doc_tokens in zip(matched_notes, note_tokens_list):
            doc_vec = self._compute_tfidf(doc_tokens, conn, num_docs=num_docs, df_cache=df_cache)
            sim = self._cosine_similarity(query_vec, doc_vec, mag_a=mag_q)
            if sim > 0.05:
                scored.append({
