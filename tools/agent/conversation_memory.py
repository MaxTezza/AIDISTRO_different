...
class ConversationMemory:
    ...
    def recall(self, query, top_k=5):
        ...
        # First pass: collect matching documents and all unique terms needed
        rows = conn.execute(
        ...
        for row in rows:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            candidate_conversations.append((row, doc_tokens))
        ...
        for note in notes:
            ...
            if not doc_tokens or query_terms.isdisjoint(doc_tokens):
                continue
            candidate_notes.append((note, doc_tokens))
            needed_terms.update(doc_tokens)

        # Load needed doc frequencies into memory in chunks of 999
        df_cache = {}
        ...
        scored = []
        for row, doc_tokens in candidate_conversations:
        ...
        for note, doc_tokens in candidate_notes:
            doc_vec = self._compute_tfidf(doc_tokens, conn, num_docs=num_docs, df_cache=df_cache)
            sim = self._cosine_similarity(query_vec, doc_vec)
            if sim > 0.05:
                scored.append({
